...
--order by子句不满足索引的最左前缀的要求，也可以用于查询排序，这是因为你的第一列被指定为一个常数



--二、该查询为索引的第一列提供了常量条件，而使用第二列进行排序，将两个列组合在一起，就形成了索引的最左前缀
...
--四、该查询使用了两中不同的排序方向，但是索引列都是正序排序的
